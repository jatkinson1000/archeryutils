...
def print_handicap_table(hcs, hc_sys, round_list, hc_dat, arrow_d=None,
    ...
    for i, round_i in enumerate(round_list):
        table[:, i+1], _ = hc_eq.score_for_round(round_i, hcs, hc_sys, hc_dat, arrow_d,
...
def handicap_from_score(score, rnd, hc_sys, hc_dat, arw_d=None,
                        int_prec=False):
    """
    ...
    if score > max_score:
        raise ValueError(f"The score of {score} provided is greater that the maximum of {max_score} for a {rnd.name}.")
    elif score <= 0.0:
        raise ValueError(f"The score of {score} provided is less than or equal to zero so cannot have a handicap.")
        ...
        if hc_sys in ['AA', 'AA2']:
            ...
            hc = -75
            dhc = 0.01
        s_max, _ = hc_eq.score_for_round(rnd, hc, hc_sys, hc_dat, arw_d, round_score_up=False)
        ...
        if int_prec:
            ...
            warnings.warn("Handicap requested for maximum score without integer precision.\n"
                          "Value returned will be handicap required for score > max_score-1.\n"
                          "This could cause issues if you are not working in integers.", UserWarning)
        return hc

    else:
        # ROOT FINDING
        def f_root(h, scr, rd, sys, hc_data, arw_dia):
            val, _ = hc_eq.score_for_round(rd, h, sys, hc_data, arw_dia, round_score_up=False)
            return val - scr
        if hc_sys in ['AA', 'AA2']:
            br = [-250, 175]
        else:
            br = [-75, 300]
        sol = scipy.optimize.root_scalar(f_root, args=(score, rnd, hc_sys, hc_dat, arw_d), method='brenth',
                                         bracket=br, xtol=0.001, maxiter=50)
        hc = sol.root
        ...
        if int_prec:
            ...
            while not min_h_flag:
                hc += hstep
                sc, _ = hc_eq.score_for_round(rnd, hc, hc_sys, hc_dat, arw_d, round_score_up=True)
                if sc < score:
